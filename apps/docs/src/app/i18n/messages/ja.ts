    ...
    items: {
      ...
      select: {
        ...
        examples: {
          ...
          zones: {
            title: '長いリスト',
            text: 'すべてのタイムゾーンを地域別に並べています。200行を超えると表示範囲の行だけがレンダリングされ、アクティブな選択肢はスクリーンリーダーのためにレンダリングされたまま残ります。タッチスクリーンでは、文字を入力してジャンプする代わりに、リストの上の検索フィールドを使えます。',
          },
        },
        api: {
          NuiSelect: {
            ...
            members: {
              ...
              focus: 'ボタンにフォーカスします。',
              search:
                '選択肢の上に表示する検索フィールド。常に表示する、表示しない、タッチスクリーンで選択肢が20個を超えるリストに表示する <code>auto</code> のいずれか。タッチスクリーンには、文字を入力してジャンプするためのキーがないためです。',
            },
          },
          NuiOption: {
            summary:
      ...
      'phone-field': {
        ...
        examples: {
          basic: {
            title: '電話番号',
            text: '<code>+</code> と国番号から入力すると、入力に合わせて国が切り替わります。タッチスクリーンでは、国の一覧に検索フィールドがあり、国名や国番号で探せます。',
          },
          countries: {
            title: '国の絞り込み',
        ...
        blocks: [
          ...
          },
          {
            ...
            rows: [
              ...
              ['Android の Chrome', '124', 'Android エミュレーター（タッチキーボードで入力）'],
            ],
          },
          {
            kind: 'p',
          ...
          {
            kind: 'p',
            html: 'タッチスクリーンではホバーできないため、ホバーカードはページ上の内容を補足するだけのものです。欠かせない情報はホバーカードに入れないでください。スマートフォンでは、チャットの Return キーで改行し、送信はボタンで行います。また、文字を入力してジャンプするためのキーがないので、長いリストのセレクトには検索フィールドが付きます。',
          },
