    ...
    items: {
      ...
      grid: {
        ...
        examples: {
          ...
          cards: {
            title: '手機上的卡片',
            text: '<code>layout="list"</code> 以卡片顯示列，<code>auto</code> 則在寬度小於 36rem 時這樣顯示。卡片沒有標題列，因此由工具列來排序和全選，工具列的「Filter」按鈕會為你所選的欄開啟欄面板。<code>toolbar</code> 也能讓工具列顯示在表格上方，或是不顯示。',
          },
        },
        api: {
          NuiGrid: {
            ...
            members: {
              ...
              toolbar:
                '對列排序、篩選和全選的工具列：一律、從不，或 <code>auto</code>（顯示卡片時出現）。',
        ...
        notes: [
          ...
          '卡片沒有標題列，因此工具列排在最前面：一個名為「Sort and filter」的群組，內含一個有標籤的下拉式選單，用來選擇依哪一欄排序；一組切換按鈕，用來選擇排序方向；多選時還有一個「Select all rows」核取方塊；以及一個「Filter」按鈕，用來開啟欄面板，在面板中用下拉式選單選擇欄。表格在 Tab 鍵順序中所佔的位置會移到第一張卡片上。',
        ],
      },
      chat: {
        name: '聊天',
